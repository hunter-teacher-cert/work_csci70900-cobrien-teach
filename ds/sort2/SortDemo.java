    ...
    private ArrayList<Integer> merge(ArrayList<Integer> a, ArrayList<Integer> b){
            ...
            while ((i+j) < (a.size()+b.size()  - 1) ) {
              ...
              if (a.get(i)<=b.get(j)){
                ...
                if (i < a.size() -1){ ////making sure that index is not off the a list
                  i++;  //increase index in a list
                }//end if
              }else {
                System.out.println("b.get(i) = " + b.get(i));
                result.add(b.get(j));   //add element from b list to the resulting list
                if (j < b.size()-1){ //making sure that index is not off the b list
